...
- Repo `FastAvan/Wheelp` es público: nunca secretos en código, `Info.plist` o git. CI/CD despliega a TestFlight solo si pasan build, tests y la suite de autorización contra Postgres.
- Verificación de identidad de ayudantes vía Didit en el dispositivo; Wheelp solo guarda el `kyc_session_id`.
- La app trata datos de categoría especial (discapacidad) y ubicación aproximada; cualquier incidente que los toque es candidato a notificación bajo RGPD — escala a `wheelp-legal` antes de comunicar nada externamente.
